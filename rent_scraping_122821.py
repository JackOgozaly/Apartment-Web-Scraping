...
#%%
#Only select correctly formatted rows
correct_format_rows = df3[df3[11] == "Floor"]
correct_format_rows = df3[df3[14] != "Top"]

if ~misc_floor.empty & ~available_df.empty:
    df3 = pd.concat([available_df,misc_floor,correct_format_rows])
if misc_floor.empty & available_df.empty:
    df3 = correct_format_rows
if ~misc_floor.empty & available_df.empty:
    df3 = pd.concat([misc_floor,correct_format_rows])
if misc_floor.empty & ~available_df.empty:
    df3 = pd.concat([available_df,correct_format_rows])
...
cols = [1,3,6,8,12,14]
df = df3[df3.columns[cols]]
df.columns= ['lease_term', "Beds",'Baths','sq.ft','Floor','Move_in_date']
...
test = pd.concat([available_df,misc_floor,correct_format_rows])
...
#%%
#Clean up our dataframe
df =df[~df['Floor'].str.contains("Floor")]
df =df[~df.Floor.str.contains("Level")]
df =df[~df.Floor.str.contains("Available")]
df =df[~df.Floor.str.contains("/")]
...
df['Amenity'] = df['Amenity'].str.split()
df = df.drop(['Misc'], axis=1)
df = df[['Price', 'Beds', 'Baths', 'sq.ft', 'Floor', 'Move_in_date', 'Amenity',
       'URL', 'building_id', 'unit_id']]
