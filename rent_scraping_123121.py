# -*- coding: utf-8 -*-
"""
Created on Fri Dec 31 13:31:46 2021
...
def special_offer_cleaner(string): 
    ...
    string = string.replace('<div class="special-offer">', '')
    string = string.replace('Special Offer <i class="fa fa-usd special-icon" popover=', '').strip()
    string = string.replace('popover-placement="right" popover-trigger="mouseenter"></i>', '')
    string = string.replace('</div>', '').strip()
    return(string)
  ...
  for j in range(0,4):
      ...
      if results is None:
         ...
         for job_elem in job_elems:
           ...
           #Special offer calc
           special_offer = results.find('div', class_='special-offer')
           if special_offer is None: 
               special_offer = np.NAN
           else: 
               #Use our function to extract the text we need
               special_offer = special_offer_cleaner(special_offer)
           x= title_elem.text.strip()
           y= company_elem.text.strip()
           z= other_amenity.text.strip()
           x1 = special_offer
           combs1.append((x, y,z, x1))
           
           
         df2 = pd.DataFrame(combs1)
         if df2.empty:
             continue
         df2.columns = ['Price', 'Misc', 'Amenity', 'Special_offer']
         df2["URL"]= URL
...
df = df[['Price', 'lease_term','Beds', 'Baths', 'sq.ft', 'Floor', 'Move_in_date', 'Amenity',
       'URL', 'building_id', 'unit_id', 'Special_offer' ]]
...
df.to_csv('Rent_Data_Scrape4.csv', mode='a', index=False, header=False)
